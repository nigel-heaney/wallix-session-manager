...
funcs='''
<script>	
	function launch(servername,wallixid,serverenv,product,domain,u,os) {
		var request = new XMLHttpRequest();
		var url = '/cgi-bin/launch.py?servername=' + servername + '&wid=' + wallixid + '&u=' + u + '&type=' + serverenv + '&p=' + product + '&d=' + domain + '&os=' + os;
		
		request.open("GET", url, true);
		request.send(null);
	}
	function launchsftp(servername,wallixid,serverenv,product,domain,u,os) {
		var request = new XMLHttpRequest();
		var url = '/cgi-bin/launchsftp.py?servername=' + servername + '&wid=' + wallixid + '&u=' + u + '&type=' + serverenv + '&p=' + product + '&d=' + domain + '&os=' + os;
		request.open("GET", url, true);
		request.send(null);
	}
</script>
...
query = "SELECT ServerEnvironment,ServerUsage,NATIP,Servername,DataCentre,Domain,ServerDescription,mainip,site,wallixid,Product,ServerOS FROM ASPServer where Customer like '"
query += customer + "' AND Deleted = 'N' order by ServerEnvironment desc, ServerUsage;"
...
print "<table><tr><th>Options</th><th>Server Name</th><th>Main IP</th><th>NAT</th><th>Type</th><th>Function</th><th>Description</th><th>DC</th><th>Domain</th><th>SiteID</th><th>WallixID</th></tr>"
